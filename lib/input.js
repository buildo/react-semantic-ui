    ...
    RadioGroup: {
      ...
      renderInput: function(props) {
        var self = this,
            ...
            options = exports.optionsRetriever.call(this, value).map(function(option, i) {
              var id = common.uniqueId(),
                  className = common.mergeClassNames('ui radio checkbox', props.className, this._className);
              return React.DOM.div({keu: i, className: 'field'}, React.DOM.div({className: className},
                React.DOM.input({id: id, type: 'radio', name: props.name, value: option.value,
                    checked: option.selected, onChange: common.eventBinder(option.value, 'onChange', self)}),
